...
export default async function SupervisorProfilePage() {
  const sessionUser = await requireUser(["SUPERVISOR"]);
  const user = await prisma.user.findUniqueOrThrow({ where: { id: sessionUser.id } });
  return (
    <div className="animate-fade-up">
      <PageHeader title="Profile" description="Update your academic profile details." />
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <p className="mb-4 text-sm text-slate-500">
            ...
            {user.staffId ? ` · Staff ID: ${user.staffId}` : ""}
          </p>
